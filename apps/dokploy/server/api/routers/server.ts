...
export const serverRouter = createTRPCRouter({
    ...
    security: protectedProcedure
        ...
        .query(async ({ input, ctx }) => {
            try {
                ...
                return response as unknown as {
                    ufw: { installed: boolean; active: boolean; defaultIncoming: string; };
                    ssh: { enabled: boolean; keyAuth: boolean; permitRootLogin: string; passwordAuth: string; usePam: string; };
                    nonRootUser: { hasValidSudoUser: boolean; };
                    unattendedUpgrades: { installed: boolean; active: boolean; updateEnabled: number; upgradeEnabled: number; };
                    fail2ban: { installed: boolean; enabled: boolean; active: boolean; sshEnabled: string; sshMode: string; };
                };
            } catch (error) {
                throw new TRPCError({
                    code: "BAD_REQUEST",
    ...
    getServerMetrics: protectedProcedure
        ...
        .query(async ({ input }) => {
            try {
                ...
                return data as {
                    cpu: string; cpuModel: string; cpuCores: number; cpuPhysicalCores: number; cpuSpeed: number; os: string; distro: string; kernel: string; arch: string; memUsed: string; memUsedGB: string; memTotal: string; uptime: number; diskUsed: string; totalDisk: string; networkIn: string; networkOut: string; timestamp: string;
                }[];
            } catch (error) {
                throw error;
            }
    ...
    terminal: protectedProcedure
        ...
        .subscription(async ({ input, ctx }) => {
            ...
            return observable<string>((emit) => {
                ...
                // Map this shell instance to the server ID (or user session)
                terminals.set(input.serverId, shell);
                ...
                shell.onExit(() => {
                    terminals.delete(input.serverId);
                    emit.complete();
                });
    ...
    terminalWrite: protectedProcedure
        .input(z.object({ serverId: z.string(), data: z.string() }))
        .mutation(async ({ input }) => {
            const shell = terminals.get(input.serverId);
            if (shell) {
    ...
    terminalResize: protectedProcedure
        .input(z.object({ serverId: z.string(), cols: z.number(), rows: z.number() }))
        .mutation(async ({ input }) => {
            const shell = terminals.get(input.serverId);
            if (shell) {
                shell.resize(input.cols, input.rows);
                return { success: true };
            }
            return { success: false };
        }),
    // --- TERMINAL MODIFICATIONS END ---
});
