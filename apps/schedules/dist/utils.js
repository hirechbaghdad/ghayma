import { cleanUpDockerBuilder, cleanUpSystemPrune, cleanUpUnusedImages, findBackupById, findScheduleById, findServerById, findVolumeBackupById, keepLatestNBackups, runCommand, runComposeBackup, runMariadbBackup, runMongoBackup, runMySqlBackup, runPostgresBackup, runVolumeBackup, } from "@dokploy/server";
import { db } from "@dokploy/server/dist/db";
import { backups, schedules, server, volumeBackups, } from "@dokploy/server/dist/db/schema";
import { and, eq } from "drizzle-orm";
import { logger } from "./logger.js";
import { scheduleJob } from "./queue.js";
export const runJobs = async (job) => {
    try {
        if (job.type === "backup") {
            const { backupId } = job;
            const backup = await findBackupById(backupId);
            const { databaseType, postgres, mysql, mongo, mariadb, compose, backupType, } = backup;
...
export const initializeJobs = async () => {
    ...
    const filteredSchedulesBasedOnServerStatus = schedulesResult.filter((schedule) => {
        if (schedule.server) {
    ...
    logger.info({ Quantity: filteredSchedulesBasedOnServerStatus.length }, "Schedules Initialized");
    ...
    const filteredVolumeBackupsBasedOnServerStatus = volumeBackupsResult.filter((volumeBackup) => {
    ...
    logger.info({ Quantity: filteredVolumeBackupsBasedOnServerStatus.length }, "Volume Backups Initialized");
};
