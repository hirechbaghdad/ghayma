...
import { loadOrCreateConfig, removeTraefikConfig, writeTraefikConfig, } from "./application.js";
export const updateServerTraefik = (user, newHost) => {
    const { https, certificateType } = user || {};
    const appName = "dokploy";
    const config = loadOrCreateConfig(appName);
...
export const updateLetsEncryptEmail = (newEmail) => {
    try {
        if (!newEmail)
            return;
...
export const writeMainConfig = (traefikConfig) => {
    try {
        const { MAIN_TRAEFIK_PATH } = paths();
        const configPath = join(MAIN_TRAEFIK_PATH, "traefik.yml");
