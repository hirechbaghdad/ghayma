import { createServiceConfig, loadOrCreateConfig, loadOrCreateConfigRemote, removeTraefikConfig, removeTraefikConfigRemote, writeTraefikConfig, writeTraefikConfigRemote, } from "./application.js";
import { createPathMiddlewares, removePathMiddlewares } from "./middleware.js";
    ...
    if ((entryPoint === "websecure" && https) || !https) {
        ...
        for (const redirect of redirects) {
            ...
            if (domain.domainType === "preview") {
                middlewareName = `redirect-${appName.replace(/^preview-(.+)-[^-]+$/, "$1")}-${redirect.uniqueConfigKey}`;
            }
