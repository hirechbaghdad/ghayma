...
import { createDeploymentSchedule, updateDeployment, updateDeploymentStatus, } from "../../services/deployment.js";
import { findScheduleById } from "../../services/schedule.js";
import { scheduledJobs, scheduleJob as scheduleJobNode } from "node-schedule";
import { getComposeContainer, getServiceContainer } from "../docker/utils.js";
import { execAsyncRemote } from "../process/execAsync.js";
...
export const removeScheduleJob = (scheduleId) => {
    const currentJob = scheduledJobs[scheduleId];
    currentJob?.cancel();
};
export const runCommand = async (scheduleId) => {
    const { application, command, shellType, scheduleType, compose, serviceName, appName, serverId, } = await findScheduleById(scheduleId);
    ...
    if (scheduleType === "application" || scheduleType === "compose") {
        ...
        if (scheduleType === "application" && application) {
            const container = await getServiceContainer(application.appName, application.serverId);
        ...
        if (serverId) {
            try {
                await execAsyncRemote(serverId, `
					set -e
            ...
            try {
                ...
                await spawnAsync("docker", ["exec", containerId, shellType, "-c", command], (data) => {
        ...
        try {
            ...
            await spawnAsync("bash", ["-c", "./script.sh"], async (data) => {
                if (writeStream.writable) {
