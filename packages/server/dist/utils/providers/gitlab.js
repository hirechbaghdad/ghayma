...
import { findGitlabById, updateGitlab, } from "../../services/gitlab.js";
...
export const cloneGitlabRepository = async ({ type = "application", ...entity }) => {
    let command = "set -e;";
    const { appName, gitlabBranch, gitlabId, gitlabPathNamespace, enableSubmodules, serverId, } = entity;
...
export const getGitlabBranches = async (input) => {
    ...
    while (true) {
        const branchesResponse = await fetch(`${gitlabProvider.gitlabUrl}/api/v4/projects/${input.id}/repository/branches?page=${page}&per_page=${perPage}`, {
            headers: {
...
export const validateGitlabProvider = async (gitlabProvider) => {
    try {
        ...
        const perPage = 100; // GitLab's max per page is 100
        while (true) {
            const response = await fetch(`${gitlabProvider.gitlabUrl}/api/v4/projects?membership=true&page=${page}&per_page=${perPage}`, {
                headers: {
                    Authorization: `Bearer ${gitlabProvider.accessToken}`,
                },
            });
