...
export const buildMongo = async (mongo) => {
    const { appName, env, externalPort, dockerImage, memoryLimit, memoryReservation, cpuLimit, cpuReservation, databaseUser, databasePassword, command, mounts, replicaSets, } = mongo;
    const startupScript = `
#!/bin/bash
${replicaSets
        ? `
...
${command ?? "wait $MONGOD_PID"}`;
    ...
    const { HealthCheck, RestartPolicy, Placement, Labels, Mode, RollbackConfig, UpdateConfig, Networks, StopGracePeriod, EndpointSpec, } = generateConfigContainer(mongo);
