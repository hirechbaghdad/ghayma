...
import { getAllServers } from "../../services/server.js";
import { eq } from "drizzle-orm";
import { scheduleJob } from "node-schedule";
import { db } from "../../db/index.js";
import { startLogCleanup } from "../access-log/handler.js";
import { cleanUpDockerBuilder, cleanUpSystemPrune, cleanUpUnusedImages, } from "../docker/utils.js";
import { sendDockerCleanupNotifications } from "../notifications/docker-cleanup.js";
import { execAsync, execAsyncRemote } from "../process/execAsync.js";
import { getS3Credentials, scheduleBackup } from "./utils.js";
export const initCronJobs = async () => {
    ...
    if (admin.user.enableDockerCleanup) {
        scheduleJob("docker-cleanup", "0 0 * * *", async () => {
            console.log(`Docker Cleanup ${new Date().toLocaleString()}]  Running docker cleanup`);
    ...
    for (const server of servers) {
        ...
        if (enableDockerCleanup) {
            scheduleJob(serverId, "0 0 * * *", async () => {
                console.log(`SERVER-BACKUP[${new Date().toLocaleString()}] Running Cleanup ${name}`);
                await cleanUpUnusedImages(serverId);
                await cleanUpDockerBuilder(serverId);
                await cleanUpSystemPrune(serverId);
                await sendDockerCleanupNotifications(admin.user.id, `Docker cleanup for Server ${name} (${serverId})`);
            });
        }
    }
    ...
    for (const backup of backups) {
        try {
            if (backup.enabled) {
                ...
                console.log(`[Backup] ${backup.databaseType} Enabled with cron: [${backup.schedule}]`);
            }
        }
        catch (error) {
...
export const keepLatestNBackups = async (backup, serverId) => {
