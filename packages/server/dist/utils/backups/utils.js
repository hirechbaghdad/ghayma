...
import { keepLatestNBackups } from "./index.js";
import { runComposeBackup } from "./compose.js";
import { runMariadbBackup } from "./mariadb.js";
import { runMongoBackup } from "./mongo.js";
import { runMySqlBackup } from "./mysql.js";
import { runPostgresBackup } from "./postgres.js";
import { runWebServerBackup } from "./web-server.js";
export const scheduleBackup = (backup) => {
    const { schedule, backupId, databaseType, postgres, mysql, mongo, mariadb, compose, } = backup;
...
export const removeScheduleBackup = (backupId) => {
    const currentJob = scheduledJobs[backupId];
    currentJob?.cancel();
};
export const normalizeS3Path = (prefix) => {
...
export const getS3Credentials = (destination) => {
...
export const getPostgresBackupCommand = (database, databaseUser) => {
...
export const getMariadbBackupCommand = (database, databaseUser, databasePassword) => {
...
export const getMysqlBackupCommand = (database, databasePassword) => {
...
export const getMongoBackupCommand = (database, databaseUser, databasePassword) => {
...
export const getServiceContainerCommand = (appName) => {
    return `docker ps -q --filter "status=running" --filter "label=com.docker.swarm.service.name=${appName}" | head -n 1`;
};
export const getComposeContainerCommand = (appName, serviceName, composeType) => {
...
const getContainerSearchCommand = (backup) => {
    const { backupType, postgres, mysql, mariadb, mongo, compose, serviceName } = backup;
    if (backupType === "database") {
        const appName = postgres?.appName || mysql?.appName || mariadb?.appName || mongo?.appName;
...
export const generateBackupCommand = (backup) => {
    ...
    switch (databaseType) {
        ...
        case "mysql": {
            ...
            if (backupType === "database" && mysql) {
                return getMysqlBackupCommand(backup.database, mysql.databaseRootPassword);
            }
            if (backupType === "compose" && backup.metadata?.mysql) {
                return getMysqlBackupCommand(backup.database, backup.metadata?.mysql?.databaseRootPassword || "");
            }
            break;
        }
        case "mariadb": {
            ...
            if (backupType === "database" && mariadb) {
                return getMariadbBackupCommand(backup.database, mariadb.databaseUser, mariadb.databasePassword);
            }
            if (backupType === "compose" && backup.metadata?.mariadb) {
                return getMariadbBackupCommand(backup.database, backup.metadata.mariadb.databaseUser, backup.metadata.mariadb.databasePassword);
            }
            break;
        }
        case "mongo": {
            ...
            if (backupType === "database" && mongo) {
                return getMongoBackupCommand(backup.database, mongo.databaseUser, mongo.databasePassword);
            }
            if (backupType === "compose" && backup.metadata?.mongo) {
                return getMongoBackupCommand(backup.database, backup.metadata.mongo.databaseUser, backup.metadata.mongo.databasePassword);
            }
            break;
        }
        default:
...
export const getBackupCommand = (backup, rcloneCommand, logPath) => {
