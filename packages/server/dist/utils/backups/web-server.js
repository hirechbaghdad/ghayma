...
import { findDestinationById } from "../../services/destination.js";
import { execAsync } from "../process/execAsync.js";
import { getS3Credentials, normalizeS3Path } from "./utils.js";
export const runWebServerBackup = async (backup) => {
    if (IS_CLOUD) {
        return;
    }
    ...
    try {
        ...
        try {
            ...
            const { stdout: containerId } = await execAsync(`docker ps --filter "name=dokploy-postgres" --filter "status=running" -q | head -n 1`);
            if (!containerId) {
            ...
            await execAsync(`rsync -a --ignore-errors ${BASE_PATH}/ ${tempDir}/filesystem/`);
