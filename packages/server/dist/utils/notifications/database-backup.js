...
import DatabaseBackupEmail from "../../emails/emails/database-backup.js";
...
export const sendDatabaseBackupNotifications = async ({ projectName, applicationName, databaseType, type, errorMessage, organizationId, databaseName, }) => {
    const date = new Date();
    const unixDate = ~~(Number(date) / 1000);
    const notificationList = await db.query.notifications.findMany({
        where: and(eq(notifications.databaseBackup, true), eq(notifications.organizationId, organizationId)),
        with: {
            email: true,
            discord: true,
            telegram: true,
    ...
    for (const notification of notificationList) {
        ...
        try {
            if (email) {
                const template = await renderAsync(DatabaseBackupEmail({
                    projectName,
                    applicationName,
                    databaseType,
                    type,
                ...
                await sendEmailNotification(email, "Database backup for dokploy", template);
            }
            if (discord) {
            ...
            if (gotify) {
                ...
                await sendGotifyNotification(gotify, decorate(type === "success" ? "✅" : "❌", `Database Backup ${type === "success" ? "Successful" : "Failed"}`), `${decorate("🛠️", `Project: ${projectName}`)}` +
            ...
            if (ntfy) {
                await sendNtfyNotification(ntfy, `Database Backup ${type === "success" ? "Successful" : "Failed"}`, `${type === "success" ? "white_check_mark" : "x"}`, "", `🛠Project: ${projectName}\n` +
