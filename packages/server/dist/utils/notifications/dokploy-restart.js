...
import DokployRestartEmail from "../../emails/emails/dokploy-restart.js";
...
export const sendDokployRestartNotifications = async () => {
    ...
    for (const notification of notificationList) {
        ...
        try {
            if (email) {
                const template = await renderAsync(DokployRestartEmail({ date: date.toLocaleString() })).catch();
                await sendEmailNotification(email, "Atlanexis CloudOS Node Restarted", template);
            }
            if (discord) {
            ...
            if (gotify) {
                ...
                await sendGotifyNotification(gotify, decorate("✅", "Atlanexis CloudOS Node Restarted"), `${decorate("🕒", `Date: ${date.toLocaleString()}`)}`);
            }
            if (ntfy) {
                await sendNtfyNotification(ntfy, "Atlanexis CloudOS node Restarted", "white_check_mark", "", `🕒Date: ${date.toLocaleString()}`);
            }
            if (telegram) {
                await sendTelegramNotification(telegram, `<b>✅ Atlanexis CloudOS node Restarted</b>\n\n<b>Date:</b> ${format(date, "PP")}\n<b>Time:</b> ${format(date, "pp")}`);
            }
            if (slack) {
                const { channel } = slack;
