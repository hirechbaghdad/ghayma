...
import { IS_CLOUD, paths } from "../../constants/index.js";
import { getS3Credentials } from "../backups/utils.js";
import { execAsync } from "../process/execAsync.js";
export const restoreWebServerBackup = async (destination, backupFile, emit) => {
    if (IS_CLOUD) {
        return;
    }
    try {
        ...
        try {
            ...
            await execAsync(`rclone copyto ${rcloneFlags.join(" ")} "${backupPath}" "${tempDir}/${backupFile}"`);
            ...
            const { stdout: hasGzFile } = await execAsync(`ls ${tempDir}/database.sql.gz || true`);
            ...
            const { stdout: hasSqlFile } = await execAsync(`ls ${tempDir}/database.sql || true`);
            if (!hasSqlFile.includes("database.sql")) {
                throw new Error("Database file not found after extraction");
            }
            const { stdout: postgresContainer } = await execAsync(`docker ps --filter "name=dokploy-postgres" --filter "status=running" -q | head -n 1`);
            ...
            await execAsync(`docker exec ${postgresContainerId} psql -U dokploy postgres -c "SELECT pg_terminate_backend(pg_stat_activity.pid) FROM pg_stat_activity WHERE pg_stat_activity.datname = 'dokploy' AND pid <> pg_backend_pid();"`);
            emit("Dropping existing database...");
            await execAsync(`docker exec ${postgresContainerId} psql -U dokploy postgres -c "DROP DATABASE IF EXISTS dokploy;"`);
            emit("Creating fresh database...");
            await execAsync(`docker exec ${postgresContainerId} psql -U dokploy postgres -c "CREATE DATABASE dokploy;"`);
            // Copy the backup file into the container
            emit("Copying backup file into container...");
            await execAsync(`docker cp ${tempDir}/database.sql ${postgresContainerId}:/tmp/database.sql`);
            // Verify file in container
            emit("Verifying file in container...");
            await execAsync(`docker exec ${postgresContainerId} ls -l /tmp/database.sql`);
            // Restore from the copied file
            emit("Running database restore...");
            await execAsync(`docker exec ${postgresContainerId} pg_restore -v -U dokploy -d dokploy /tmp/database.sql`);
            // Cleanup the temporary file in the container
            emit("Cleaning up container temp file...");
            await execAsync(`docker exec ${postgresContainerId} rm /tmp/database.sql`);
