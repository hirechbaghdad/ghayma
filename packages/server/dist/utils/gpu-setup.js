...
import { execAsync, execAsyncRemote, sleep } from "../utils/process/execAsync.js";
export async function checkGPUStatus(serverId) {
    try {
        const [driverInfo, runtimeInfo, swarmInfo, gpuInfo, cudaInfo] = await Promise.all([
...
const checkGpuDriver = async (serverId) => {
    let driverVersion;
    let driverInstalled = false;
    let availableGPUs = 0;
    try {
        const driverCommand = "nvidia-smi --query-gpu=driver_version --format=csv,noheader";
        ...
        if (driverVersion) {
            ...
            const countCommand = "nvidia-smi --query-gpu=gpu_name --format=csv,noheader | wc -l";
...
const checkRuntime = async (serverId) => {
...
const checkSwarmResources = async (serverId) => {
    let swarmEnabled = false;
    let gpuResources = 0;
    try {
        const nodeCommand = "docker node inspect self --format '{{json .Description.Resources.GenericResources}}'";
        ...
        if (resources && resources !== "null") {
            ...
            for (const resource of genericResources) {
                if (resource.DiscreteResourceSpec &&
...
const checkGpuInfo = async (serverId) => {
    let gpuModel;
    let memoryInfo;
    try {
        const gpuInfoCommand = "nvidia-smi --query-gpu=gpu_name,memory.total --format=csv,noheader";
...
const checkCudaSupport = async (serverId) => {
    let cudaVersion;
...
export async function setupGPUSupport(serverId) {
    try {
        ...
        if (error instanceof Error &&
            ...
            throw new Error("Sudo access required. Please run with appropriate permissions.");
        }
        throw error;
    }
}
const validatePrerequisites = async (initialStatus) => {
    if (!initialStatus.driverInstalled) {
        throw new Error("NVIDIA drivers not installed. Please install appropriate NVIDIA drivers first.");
    }
    if (!initialStatus.runtimeInstalled) {
        throw new Error("NVIDIA Container Runtime not installed. Please install nvidia-container-runtime first.");
    }
...
const getNodeId = async (serverId) => {
...
const createDaemonConfig = (availableGPUs) => ({
    runtimes: {
        nvidia: {
...
const setupRemoteServer = async (serverId, daemonConfig) => {
    const setupCommands = [
        "sudo -n true",
...
const setupLocalServer = async (daemonConfig) => {
    ...
    try {
        ...
        throw new Error("Failed to configure GPU support. Please ensure you have sudo privileges and try again.");
    }
};
const addGpuLabel = async (nodeId, serverId) => {
...
const verifySetup = async (nodeId, serverId) => {
