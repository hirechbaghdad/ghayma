...
import { recreateDirectory, recreateDirectoryRemote, } from "../filesystem/directory.js";
import { execAsyncRemote } from "../process/execAsync.js";
export const unzipDrop = async (zipFile, application) => {
    let sftp = null;
    try {
        ...
        const rootEntries = zipEntries.filter((entry) => entry.entryName.split("/").length === 1 ||
            (entry.entryName.split("/").length === 2 &&
                entry.entryName.endsWith("/")));
        const hasSingleRootFolder = !!(rootEntries.length === 1 && rootEntries[0]?.isDirectory);
            ...
            if (application.serverId) {
                if (!entry.isDirectory) {
                    ...
                    try {
                        ...
                        await execAsyncRemote(application.serverId, `mkdir -p "${dirPath}"`);
...
const getSFTPConnection = async (serverId) => {
...
const uploadFileToServer = (sftp, data, remotePath) => {
    return new Promise((resolve, reject) => {
