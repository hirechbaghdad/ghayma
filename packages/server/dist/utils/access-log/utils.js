...
export function processLogs(logString, dateRange) {
...
export function parseRawConfig(rawConfig, page, sort, search, status, dateRange) {
    try {
        ...
        if (search) {
            parsedLogs = parsedLogs.filter((log) => log.RequestPath.toLowerCase().includes(search.toLowerCase()));
        }
        if (status && status.length > 0) {
            parsedLogs = parsedLogs.filter((log) => status.some((range) => isStatusInRange(log.DownstreamStatus, range)));
        }
        const totalCount = parsedLogs.length;
        if (sort) {
            parsedLogs = _.orderBy(parsedLogs, [sort.id], [sort.desc ? "desc" : "asc"]);
        }
        else {
...
const isStatusInRange = (status, range) => {
    switch (range) {
        case "info":
