...
import { generateBase64, generateHash, generateJwt, generatePassword, generateRandomDomain, } from "./index.js";
/**
 * Process a string value and replace variables
 */
export function processValue(value, variables, schema) {
 ...
 */
export function processVariables(template, schema) {
    const variables = {};
 ...
 */
export function processDomains(template, variables, schema) {
    if (!template?.config?.domains ||
        template.config.domains.length === 0 ||
        template.config.domains.every((domain) => !domain.serviceName)) {
        return [];
    }
    return template?.config?.domains?.map((domain) => ({
        ...domain,
        host: domain.host
 ...
 */
export function processEnvVars(template, variables, schema) {
 ...
 */
export function processMounts(template, variables, schema) {
    if (!template?.config?.mounts ||
        template.config.mounts.length === 0 ||
        template.config.mounts.every((mount) => !mount.filePath && !mount.content)) {
        return [];
    }
    return template?.config?.mounts?.map((mount) => ({
 ...
 */
export function processTemplate(template, schema) {
