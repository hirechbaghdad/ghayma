...
import { backups, buildAppName, mariadb, } from "../db/schema/index.js";
import { generatePassword } from "../templates/index.js";
import { buildMariadb } from "../utils/databases/mariadb.js";
...
export const createMariadb = async (input) => {
...
export const findMariadbById = async (mariadbId) => {
...
export const updateMariadbById = async (mariadbId, mariadbData) => {
...
export const removeMariadbById = async (mariadbId) => {
    const result = await db
        .delete(mariadb)
        .where(eq(mariadb.mariadbId, mariadbId))
        .returning();
...
export const findMariadbByBackupId = async (backupId) => {
    const result = await db
        .select({
...
export const deployMariadb = async (mariadbId, onData) => {
    ...
    try {
        ...
        if (mariadb.serverId) {
            await execAsyncRemote(mariadb.serverId, `docker pull ${mariadb.dockerImage}`, onData);
        }
        else {
