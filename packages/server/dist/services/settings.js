...
import { initializeStandaloneTraefik, initializeTraefikService, } from "../setup/traefik-setup.js";
...
export const getUpdateData = async () => {
    ...
    if (imageTag === "latest") {
        const versionedTag = allResults.find((t) => t.digest === searchedDigest && t.name.startsWith("v"));
        if (!versionedTag) {
            return DEFAULT_UPDATE_DATA;
        }
...
export const reloadDockerResource = async (resourceName, serverId) => {
...
export const readPorts = async (resourceName, serverId) => {
    ...
    for (const key in parsedResult) {
        if (Object.hasOwn(parsedResult, key)) {
            ...
            containerPortMapppings.forEach((mapping) => {
                ports.push({
