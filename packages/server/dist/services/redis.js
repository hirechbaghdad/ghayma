...
import { buildAppName, redis, } from "../db/schema/index.js";
import { generatePassword } from "../templates/index.js";
import { buildRedis } from "../utils/databases/redis.js";
...
export const createRedis = async (input) => {
...
export const findRedisById = async (redisId) => {
...
export const updateRedisById = async (redisId, redisData) => {
...
export const removeRedisById = async (redisId) => {
    const result = await db
        .delete(redis)
        .where(eq(redis.redisId, redisId))
        .returning();
...
export const deployRedis = async (redisId, onData) => {
    ...
    try {
        ...
        if (redis.serverId) {
            await execAsyncRemote(redis.serverId, `docker pull ${redis.dockerImage}`, onData);
        }
        else {
