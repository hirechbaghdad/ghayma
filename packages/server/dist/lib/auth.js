...
import { IS_CLOUD } from "../constants/index.js";
import { db } from "../db/index.js";
import * as schema from "../db/schema/index.js";
import { getUserByToken } from "../services/admin.js";
import { updateUser } from "../services/user.js";
import { getHubSpotUTK, submitToHubSpot } from "../utils/tracking/hubspot.js";
import { sendEmail } from "../verification/send-verification-email.js";
import { getPublicIpWithFallback } from "../wss/utils.js";
                    ...
                    if (IS_CLOUD || !isAdminPresent) {
                        await db.transaction(async (tx) => {
                            const organization = await tx
                                ...
                                .values({
                                name: "Atlanexis Default Organization",
                                ownerId: user.id,
                                createdAt: new Date(),
                            })
                                .returning()
...
export const validateRequest = async (request) => {
    ...
    if (apiKey) {
        try {
            ...
            const { id, name, email, emailVerified, image, createdAt, updatedAt, twoFactorEnabled, } = apiKeyRecord.user;
            const mockSession = {
                session: {
    ...
    if (session?.user) {
        const member = await db.query.member.findFirst({
            where: and(eq(schema.member.userId, session.user.id), eq(schema.member.organizationId, session.session.activeOrganizationId || "")),
            with: {
                organization: true,
            },
        });
